...
typedef struct ngx_http_condition_func_s  ngx_http_condition_func_t;
...
struct ngx_http_condition_func_s {
    ngx_str_t                          name;
    ngx_http_condition_func_pt         handler;
    ngx_condition_func_e               type;
    ngx_uint_t                         min_args;
    ngx_uint_t                         max_args;
...
static ngx_http_condition_func_t *ngx_http_condition_find_func(
...
static ngx_http_condition_func_t  ngx_http_condition_funcs[] = {

    { ngx_string("not"),
      ngx_http_condition_logic_handler,
    ...
    { ngx_string("bool"),
      ngx_http_condition_bool_handler,
      NGX_CONDITION_FUNC_BOOL,
      1, 1,
      NGX_CONDITION_NO_IGNORE_CASE },

    { ngx_string("is_empty"),
      ngx_http_condition_string_handler,
    ...
    { ngx_string("is_num"),
      ngx_http_condition_number_handler,
    ...
    { ngx_string("time_range"),
      ngx_http_condition_time_handler,
      NGX_CONDITION_FUNC_TIME_RANGE,
      NGX_CONDITION_NO_ARGS, 9,
      NGX_CONDITION_NO_IGNORE_CASE },

    { ngx_string("is_ip"),
      ngx_http_condition_ip_handler,
...
static ngx_http_condition_func_t *
ngx_http_condition_find_func(ngx_str_t *name, ngx_uint_t *negative)
{
    ...
    *negative = 0;
    ...
    for (func = ngx_http_condition_funcs; func->name.len; func++) {
...
ngx_http_condition(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    first = 3;
    func = ngx_http_condition_find_func(&value[2], &negative);
    if (func == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
    ...
    if (func->type == NGX_CONDITION_FUNC_NOT && cf->args->nelts > 3) {
        modifier = ngx_http_condition_find_func(&value[3], &modifier_negative);
        if (modifier != NULL
    ...
    if (entry != NULL && entry->type != func->type) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           ...
                           &value[1],
                           &ngx_http_condition_funcs[entry->type].name,
                           &func->name);
        return NGX_CONF_ERROR;
    }
    ...
    ngx_http_condition_def_t *definition, ngx_uint_t depth)
{
    ...
    if (definition->func->type == NGX_CONDITION_FUNC_STR_REGEX_MATCH) {
        ...
        return ngx_http_condition_apply_negation(definition, result >= 0);
#else
        return 0;
#endif
    }
    ...
    ngx_command_t *cmd, void *conf)
{
    ...
    element_size = sizeof(ngx_http_condition_complex_value_ctx_t);
    value_offset = offsetof(ngx_http_condition_complex_value_ctx_t, value);
    expr_id_offset = offsetof(ngx_http_condition_complex_value_ctx_t, expr_id);
